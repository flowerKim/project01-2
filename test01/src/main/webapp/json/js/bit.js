function bit(value) {
	...
	if (value instanceof HTMLElement) {
		...
		obj = [document.createElement(value.substring(1, value.length - 1))];
	} else {
	...
	obj.click = function(listener) {
		...
		for (var i = 0; i < this.length; i++) {
			if (this[i].addEventListener) { // 크롬, 사파리, 파폭, IE9
	...
	obj.val = function(value) {
		if (arguments.length == 0) { // getter역할을 수행한다.
	...
	obj.css = function(propertyName, value) {
		if (arguments.length == 1) { // getter역할을 수행한다.
			// 값을 리턴할 때는 첫 번째 엘리먼트의 값만 리턴한다.
			// jQuery 작동 방식이 그러함.
...
bit.toQueryString = function(obj) {
	...
	for (var propName in obj) {
		...
		queryString += propName + '=' + encodeURIComponent(obj[propName]);
	}
	return queryString;
};
